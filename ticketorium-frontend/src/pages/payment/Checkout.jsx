...
function Checkout(props) {
    ...
    // Price and eventId passed from JoinModal via navigate("/checkout", { state: { ... } })
    const { price = 0, eventId = null } = location.state || {};


    useEffect(() => {
        if (eventId !== null) {
            console.log("Checkout for event:", eventId, "with price:", price);
            // TODO later: verify price against server, track analytics, etc.
        }
    }, [eventId, price]);
    ...
    function handleCheckout(e) {
        ...
        if (expiryDate.trim() === "") {
            ...
            parseInt(expiryDate.slice(3, 5)) < currentTime.getFullYear() % 100 ||
            (parseInt(expiryDate.slice(3, 5)) === currentTime.getFullYear() % 100 &&
                parseInt(expiryDate.slice(0, 2)) < currentTime.getMonth() + 1)
        ) {
        ...
        if (Object.keys(errorsFound).length > 0) {
            ...
            // Demo: 4/10 chance of payment failure
            if (Math.random() < 0.4) {
            ...
            // Optionally pass eventId / price to outcome as well later
    ...
    const handleChangeNumber = (e) => {
        ...
        setCardType(
            cleaned.startsWith("34") || cleaned.startsWith("37")
                ? "amex"
    ...
    const handleChangeDate = (e) => {
        ...
        // whether the year is valid or not is checked at handleCheckout
    ...
    return (
        <div className="h-screen w-full flex justify-center content-center">
            <div className="lg:w-[50%] xl:w-[44%] m-10 lg:m-20 flex flex-col gap-3">
                <h2 className="font-[Epilogue-Black] text-[50px] xl:text-[60px] text-[var(--primary-color)]">
                    Checkout
                </h2>

                <div
                    id="price-and-card-options"
                    className="flex justify-between items-center"
                >
                    <img
                        src="src/assets/images/payment/card_types.svg"
                        className="h-13"
                    />
                    <span className="font-[Gilroy-Bold] text-[var(--secondary-color)] text-[22px]">
                ...
                <form className="flex flex-col items-center gap-3">
                    <PaymentInput
                        option="name-on-card"
                        value={cardName}
                        updater={handleChangeName}
                        errors={errors}
                    />
                    <PaymentInput
                        option="card-number"
                        value={cardNumber}
                        updater={handleChangeNumber}
                        errors={errors}
                        cardType={cardType}
                    />
                    <div className="flex gap-3 w-full">
                        <PaymentInput
                            option="expiry-date"
                            value={expiryDate}
                            updater={handleChangeDate}
                            errors={errors}
                        />
                        <PaymentInput
                            option="cvv"
                            value={cvv}
                            updater={handleChangeCVV}
                            errors={errors}
                        />
                    </div>
                    <button
                        onClick={handleCheckout}
                        className="cursor-pointer bg-[var(--accent-color)] font-[Epilogue-Bold] rounded-[0.1em] w-full h-[62px] mt-4 text-[var(--primary-black)]"
                    >
                        Complete Purchase
                    </button>
                </form>
                ...
                <img
                    src={"src/assets/images/payment/powered_by_stripe.svg"}
