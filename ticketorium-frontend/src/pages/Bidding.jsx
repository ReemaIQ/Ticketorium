...
// NEW: use the tickets API helper we added
import { fetchUnlistedTicketsForUser } from "../api/tickets";

// --- Helpers ---------------------------------------------------------

// Normalize "user" into a string id for comparisons / sellerId
...
function listingToBidding(l, currentUserId = null) {
    ...
    // whether current user has placed a bid (if we know currentUserId)
    const hasUserBid = currentUserId
        ? topBids.some((tb) => tb.bidder === currentUserId)
        : false;
...
function transformListingsToBiddings(listings, currentUserId) {
    const map = {};
    listings.forEach((l) => {
        map[l._id || l.id] = listingToBidding(l, currentUserId);
    });
    return map;
}
...
export default function Bidding({ user }) {
    ...
    // normalize the incoming `user` prop to a string id (for hasUserBid, sellerId)
    const currentUserId = normalizeUserId(user);

    console.log("[debug] Bidding user prop:", user);
    console.log("[debug] normalized currentUserId:", currentUserId);
    ...
    const loadListings = useCallback(async () => {
        try {
            ...
            // use currentUserId so hasUserBid works even if `user` is an object
            setBiddings(transformListingsToBiddings(listings, currentUserId));
        } catch (err) {
            console.error(err);
        } finally {
            setLoading(false);
    ...
    const loadUnlistedTickets = useCallback(async () => {
        try {
            ...
            // use the helper that knows how to derive handle / id
            const tickets = await fetchUnlistedTicketsForUser(user);

            // ensure _id is always a string (for comparisons in modal)
    ...
    const handleCreate = async ({ ticketId, deadline, startingBid }) => {
        try {
            const sellerId = currentUserId; // normalized id string

            if (!sellerId) {
                alert("Cannot create listing: missing seller id.");
                return;
            }
            ...
            setBiddings((prev) => ({
                ...prev,
                [createdListing._id]: listingToBidding(
                    createdListing,
                    currentUserId,
                ),
            }));
