...
function App() {
    ...
    const refreshUserData = async () => {
        ...
        if (userData.university) {
            const uniResponse = await fetch(
                "http://localhost:4000/api/universities/" +
                encodeURIComponent(userData.university),
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
    ...
    const refreshEventsData = async () => {
        ...
        const uniEventsResponse = await fetch(
            "http://localhost:4000/api/events/uni-all/" +
            encodeURIComponent(university["_id"]),
            {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
