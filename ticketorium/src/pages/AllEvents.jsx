...
function AllEvents(props) {
    ...
    // local state now fetches from backend instead of relying on initial dummy props

    const [events, setEvents] = useState({}); // map of id -> event object (raw from backend)

    const [eventsJoined, setEventsJoined] = useState([]); // array of registrations
    ...
    useEffect(() => {
        ...
        const fetchEvents = async () => {
            ...
            try {
                ...
                // Try sending `university` query param (backend accepts `university` or `universityCode`)

                const q = props.uni ? `?university=${encodeURIComponent(props.uni)}` : "";
                ...
                // convert array to map by _id for easier usage (matches prior code expectations)
        ...
        const fetchRegistrations = async () => {
            ...
            try {
                ...
                const res = await fetch(`/api/event-registrations?user=${encodeURIComponent(props.user)}`);

                if (!res.ok) {

                    console.error("Failed to fetch event registrations:", res.statusText);
    ...
    }, [props.uni, props.user]);

    // 1) Initial: build a map of all events (merged with state)

    useEffect(() => {

        // Use the locally fetched `events` and `eventsJoined` (not props.*)

        props.filterContent(

            "initial",

            // Pass BOTH events and eventsJoined

            { events: events, eventsJoined: eventsJoined },
            ...
            // Pass loggedInUser (props.user) so we can merge 'joined'/'invited' states
    ...
    }, [events, eventsJoined, props.filterContent, props.uni, props.user]);
