...
const contentOptions = {
    ...
    organizer: {
        // reema: ORGANIZER HOME : shows analytics widget and upcoming events
        analytics: {
...
function UserHome(props) {
                ...
                return (
                    <SearchBtn
                        filterFunc={(searchValue) => {
                            props.filterContent(
                                "search",
                                upcomingEventsOriginalState.current,
                ...
                return (
                    <SearchBtn
                        filterFunc={(searchValue) => {
                            props.filterContent(
                                "search",
                                invitesReceivedOriginalState.current,
                                setFilteredInvitesReceived,
                                "event",
                                searchValue,
                                {
                                    "list-type": "invites-received",
                                    university: props.uni,
                                }
                            );
                        }}
                ...
                return (
                    <SearchBtn
                        filterFunc={(searchValue) => {
                            props.filterContent(
                                "search",
                                invitesSentOriginalState.current,
                                setFilteredInvitesSent,
                                "event",
                                searchValue,
                                {
                                    "list-type": "invites-sent",
                                    university: props.uni,
                                }
                            );
                        }}
    ...
    return (
        <>
                ...
                <div id="a" className="flex justify-between order-2 xl:order-1">
                    ...
                    >
                        ...
                        <p className="max-xl:flex flex-col items-center font-[DM-Sans-Light] text-[24px] text-white mt-7">
                            All{" "}
                            {userType === "visitor" || userType === "system-admin"
                                ? "this"
                                : "your"}{" "}
                ...
                <div className="flex max-xl:justify-between max-xl:grid max-xl:grid-cols-2 gap-25 xl:gap-15 w-full">
                    {Object.keys(contentOptions[userType]).map((key) => (
                        <span
                            ...
                            onClick={() =>
                                window.scrollTo({
                                    top: document.getElementById(key).offsetTop - 30,
                                    behavior: "smooth",
                                })
                            }
                        >
                    ...
                    <React.Fragment key={key}>
                        ...
                        >
                            ...
                            <div className="flex flex-col items-start gap-4 w-full">
                                <h2
                                    id={key}
                                    className="font-[Epilogue-Black] text-[50px] xl:text-[50px] text-[var(--primary-color)]"
                                >
                        ...
                        <div className="flex w-full max-w-6xl">
                            {key === "notifications" ? (
                                ...
                                // reema: ORGANIZER ANALYTICS widget
                                // Analytics component can read from dummyEvents / eventsJoined
                                // (depending on how you implemented it), and shows performance stats.
                                <div className="w-full max-w-6xl px-15">
                                    <Analytics />
                                </div>
                            ) : key === "event-organizers" ? (
                                <h1 className="font-[Gilroy-Medium] text-[20px]">
                                    event-organizers
                                </h1>
                            ) : key === "user-events" ? (
                                <EventList
                                    eventsJoined={props.eventsJoined}
                                    events={upcomingEventsOriginalState.current}
                                ...
                                <EventList
                                    eventsJoined={props.eventsJoined}
                                    events={invitesReceivedOriginalState.current}
                                    filteredEvents={filteredInvitesReceived}
                                    filterContent={props.filterContent}
                                    userType={userType}
                                ...
                                <EventList
                                    eventsJoined={props.eventsJoined}
                                    events={invitesSentOriginalState.current}
                                    filteredEvents={filteredInvitesSent}
                                    filterContent={props.filterContent}
                                    userType={userType}
                                    listType="invites-sent"
                                />
                            ) : key === "upcoming-events" ? (
                                // reema: Organizer's upcoming events area.
                                // Uses EventList → EventPage with Edit, Verify, QR tickets.
                                <EventList
                                    eventsJoined={props.eventsJoined}
                                    events={upcomingEventsOriginalState.current}
                                    filteredEvents={filteredUpcomingEvents}
            ...
            {/* Magic / test area (still points to Checkout demo) */}
            <NavLink
                to="/checkout"
                onClick={() => props.setIsPurchasing(true)}
                className="border-7 rounded-full py-3 px-15 m-10 self-center border-purple-200 cursor-pointer"
            >
                Magic Button
            </NavLink>
