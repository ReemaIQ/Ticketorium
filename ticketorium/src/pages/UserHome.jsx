...
function UserHome(props) {
    ...
    return (
        <>
            <button onClick={async () => {console.log("towel"); const response = await fetch("http://localhost:4000/checkout", {method: "POST"}); const data = await response.json(); console.log(data.url); window.location.href = data.url;}} className="bg-indigo-500 p-10 rounded-full text-white cursor-pointer">Test Checkout Btn</button>
            <div className=" m-0 py-10 text-3xl flex flex-col xl:flex-row bg-[var(--secondary-color)] w-full relative xl:justify-between xl:items-center xl:content-center">
                <div id="a" className="flex justify-between order-2 xl:order-1">
                    <div id="b" className="xl:px-20 xl:py-30 px-10 py-10 max-xl:flex max-xl:flex-col max-xl:w-full">
                        <h1 className="flex flex-col items-center max-xl:text-center text-[86px] sm:text-[110px] md:text-[130px] font-bold font-[Epilogue-Black] md:leading-[125px] text-white">Welcome<br/>Back,<br/>{props.firstName}!</h1>
                        <p className="max-xl:flex flex-col items-center font-[DM-Sans-Light] text-[24px] text-white mt-7">All {(props.role === "visitor" || props.role === "system-admin")? "this": "your"} university's events in one place.</p>
                    </div>
                </div>

                <img src={"/src/assets/images/home-main/unis/" + props.uni["logo"]} className="max-md:w-[40%] md:max-lg:w-[30%] lg:max-xl:w-[25%] xl:h-150 2xl:h-180 order-1 xl:order-2 self-center object-contain xl:max-w-2xl"/>
            </div>
            ...
            <div className="bg-[#F3F3F3] xl:h-[96px] w-full xl:flex xl:flex-row xl:items-center px-12 py-9 gap-12">
                ...
                <div className="flex max-xl:justify-between max-xl:grid max-xl:grid-cols-2 gap-25 xl:gap-15 w-full">
                    {Object.keys(contentOptions[props.role]).map((key) =>
                        <span className="font-[Gilroy-Medium] text-[20px] text-[var(--primary-color)] self-center text-center cursor-pointer" onClick={() => {window.scrollTo({top: document.getElementById(key).offsetTop - 30, behavior:'smooth'})}}>{contentOptions[props.role][key]["jump-to"]}</span>
                    )}
                </div>
            </div>
                    ...
                    <>
                        <div id="section-header" className="flex items-center justify-between w-full mt-9 mb-3">
                            ...
                            <div className="flex flex-col items-start gap-4 w-full">
                                <h2 id={key} className="font-[Epilogue-Black] text-[50px] xl:text-[50px] text-[var(--secondary-color)]">{contentOptions[props.role][key]["header"]}</h2>
                                    ...
                                    (key=== "analytics" ?
                                        ...
                                        (key === "upcoming-events" ?
                                            ...
                                            (key === "universities" ?
                                                    ...
                                                    (key === "user-events" ?
                                                        (
                                                            <EventList eventsJoined={props.eventsJoined} setOrganizerViewing={props.setOrganizerViewing} events={upcomingEventsOriginalState.current} filteredEvents={filteredUpcomingEvents} filterContent={props.filterContent} userType={props.role} listType="my-events"/>
                                                        )
                                                        :
                                                        (key === "invites-received" ?
                                                                <EventList eventsJoined={props.eventsJoined} setOrganizerViewing={props.setOrganizerViewing} events={invitesReceivedOriginalState.current} filteredEvents={filteredInvitesReceived} filterContent={props.filterContent} userType={props.role} listType="invites-received"/>
                                                                :
                                                                    "")
                                                        )
                                            )))
